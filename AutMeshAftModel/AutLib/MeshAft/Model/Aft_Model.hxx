...
namespace AutLib
{
	...
	public:
		...
		void LoadBoundaryEdges(const std::vector<std::shared_ptr<frontType>>& theBoundaryEdges)
		{
			theBoundaryEdges_ = theBoundaryEdges;
		}

		void LoadBoundaryMetricMap(const std::shared_ptr<metricPrcsr>& theSizeMap)
		{
		...
		static void ActiveFront(const std::vector<std::shared_ptr<frontType>>& theEdges);
