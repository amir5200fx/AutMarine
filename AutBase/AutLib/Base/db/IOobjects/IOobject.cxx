...
const ::AutLib::word AutLib::IOobject::typeName(AutLib::IOobject::typeName_());
::AutLib::debug::debugSwitch AutLib::IOobject::debug(std::string(AutLib::IOobject::typeName_()), 0, "");
...
bool AutLib::IOobject::IOobject::fileNameComponents
(
	const fileName& path,
	fileName& instance,
	fileName& local,
...
const AutLib::objectRegistry& AutLib::IOobject::db() const
{
	return db_;
}


const AutLib::Time& AutLib::IOobject::time() const
{
	return db_.time();
}


const AutLib::fileName& AutLib::IOobject::caseName() const
{
	return time().caseName();
}


const AutLib::fileName& AutLib::IOobject::rootPath() const
{
	return time().rootPath();
}


AutLib::fileName AutLib::IOobject::path() const
{
	return rootPath() / caseName() / instance() / db_.dbDir() / local();
}


AutLib::fileName AutLib::IOobject::path
(
	const word& instance,
	const fileName& local
) const
...
AutLib::fileName AutLib::IOobject::filePath() const
{
...
AutLib::Istream* AutLib::IOobject::objectStream()
{
...
void AutLib::IOobject::setBad(const string& s)
{
	if (objState_ != GOOD)
	{
...
void AutLib::IOobject::operator=(const IOobject& io)
{
	name_ = io.name_;
