...
vector AutLib::eigenValues(const tensor& t)
{
	scalar i = 0;
	scalar ii = 0;
	scalar iii = 0;
...
vector AutLib::eigenVector(const tensor & t, const scalar lambda)
{
...
tensor AutLib::eigenVectors(const tensor& t)
{
...
vector AutLib::eigenValues(const symmTensor& t)
{
	scalar i = 0;
	scalar ii = 0;
	scalar iii = 0;
	...
	else
	{
		...
		if (mag(c) < SMALL)
		{
			const scalar disc = AutLib::max(sqr(a) - 4 * b, scalar(0));
...
vector AutLib::eigenVector(const symmTensor& t, const scalar lambda)
{
	// Construct the matrix for the eigenvector problem
...
tensor AutLib::eigenVectors(const symmTensor& t)
{
	vector evals(eigenValues(t));
...
tensor AutLib::hinv(const tensor& t)
{
...
symmTensor AutLib::hinv(const symmTensor& t)
{
	static const scalar hinvLarge = 1e10;
	static const scalar hinvSmall = 1e-10;
