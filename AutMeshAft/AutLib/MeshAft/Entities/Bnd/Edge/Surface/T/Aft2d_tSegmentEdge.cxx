...
AutLib::Aft2d_tSegmentEdge::GetTopology
(
...
)
{
	...
	if (pts.size() < 2)
	{
		FatalErrorIn("std::vector<std::shared_ptr<AutLib::Aft2d_tSegmentEdge>> AutLib::Aft2d_tSegmentEdge::GetTopology()")
...
template<>
void AutLib::Aft2d_tSegmentEdge::MergeDangles
(
	const std::vector<std::shared_ptr<AutLib::Aft2d_tSegmentEdge>>& theWire,
...
)
{
	forThose(Index, 1, theWire.size() - 1)
	{
		...
		const auto pn0 = std::dynamic_pointer_cast<Aft2d_PoleNodeTemplate<Aft2d_tCornerNode>>(e0.Node1());
		const auto pn1 = std::dynamic_pointer_cast<Aft2d_PoleNodeTemplate<Aft2d_tCornerNode>>(e1.Node0());
		...
		if (pn0 OR pn1)
		{
			...
			auto node =
				Aft2d_PoleNodeTemplate<Aft2d_tCornerNode>::MergeNodes
				(
					cn0, cn1,
					Aft2d_tSegmentNode::Merge_Alg::New);
		...
		const auto ln0 = std::dynamic_pointer_cast<Aft2d_LineNodeTemplate<Aft2d_tCornerNode>>(e0.Node1());
		const auto ln1 = std::dynamic_pointer_cast<Aft2d_LineNodeTemplate<Aft2d_tCornerNode>>(e1.Node0());

		if (ln0 OR ln1)
		{
			auto node =
				Aft2d_LineNodeTemplate<Aft2d_tCornerNode>::MergeNodes
				(
					ln0, ln1,
					Aft2d_tSegmentNode::Merge_Alg::New);
		...
		{
			auto node =
				Aft2d_tCornerNode::MergeNodes(cn0, cn1, Aft2d_tCornerNode::Merge_Alg::New);
