...
namespace AutLib
{
	...
	protected:
		...
		struct mySort
		{
			...
			(
				...
				const AftMetricPrcsr& theSizeMap
			)
			{
#if MESH_DEBUG
	...
	protected:
		...
		Standard_Boolean ChackDistCriteria(const std::vector<front_ptr>& theFronts) const;
		...
		Standard_Boolean IsNewPointCandidate(const std::vector<front_ptr>& theEffectives) const;
		...
		void AddOptimumNodeTo(std::vector<node_ptr>& theNodes);
		...
		void RetrieveEarlyNodes(std::vector<node_ptr>& theNodes) const;

		template<class U = void>
		resolvedType<is_isoType<typename SizeMap::frontType>::value, U> RetrieveLocalFrontNodes
		(
			std::vector<node_ptr>& theLocals,
			std::vector<node_ptr>& theSearching
		) const
		{
			std::vector<node_ptr> Locals;
		...
		(
			...
			std::vector<front_ptr>& theEntities
		) const
		{
		...
		resolvedType<is_isoType<typename SizeMap::frontType>::value, U> UpdateFront()
		{
			...
			frontInfo::UpdatePaires(*theMetricMap_);
		...
		resolvedType<is_anisoType<typename SizeMap::frontType>::value, U> UpdateFront()
		{
			...
			if (node->Index() IS_EQUAL frontInfo::NodeCounter() + 1)
			{
				...
				const auto effect = theMetricMap_->CalcEffectiveMetric(node->Coord(), current->Centre());
	...
	public:
		...
		(
			...
			, theNodeCalculator_(theCoordCalculator)
		{}
		...
		const std::shared_ptr<SizeMap>& MetricMap() const
		{
			return theMetricMap_;
		}

		void LoadMetricMap(const std::shared_ptr<SizeMap>& theMetricMap)
		{
